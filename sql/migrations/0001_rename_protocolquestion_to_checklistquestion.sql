...
--
-- GUARDED: this only runs where `protocol_question` still exists. A fresh install
-- creates the schema from entityDefs that already say ChecklistQuestion, and an
-- instance renamed before this migration framework existed has no row in
-- schema_migrations, so both would otherwise fail here on a table that is not there.
-- (`migrate-db.sh` is now run by both fresh-install CI jobs, which is what surfaced this.)
--
...
DO $migration$
BEGIN
    IF to_regclass('public.protocol_question') IS NULL THEN
        RAISE NOTICE 'protocol_question does not exist - rename already applied or fresh install; nothing to do.';
        RETURN;
    END IF;
    ...
    RAISE NOTICE 'ProtocolQuestion schema renamed to ChecklistQuestion.';
END
$migration$;
